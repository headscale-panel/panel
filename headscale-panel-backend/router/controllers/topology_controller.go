...
func (c *TopologyController) GetTopology(ctx *gin.Context) {
	topology, err := services.TopologyService.GetTopology()
	if err != nil {
...
func (c *TopologyController) GetTopologyWithACL(ctx *gin.Context) {
	topology, err := services.TopologyService.GetTopologyWithACL()
	if err != nil {
		serializer.Fail(ctx, err)
		return
	}
...
func (c *TopologyController) GetACLMatrix(ctx *gin.Context) {
	matrix, err := services.TopologyService.GetACLMatrix()
	if err != nil {
		serializer.Fail(ctx, err)
		return
	}
