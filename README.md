...
### TLS Mode Selection (Setup/Settings)

The panel supports three gRPC connection modes. Choose based on how your Headscale gRPC endpoint is deployed:

1. **Allow insecure gRPC (No TLS)**
  - Use when Headscale gRPC is plain TCP.
  - Setup: `Enable TLS = OFF`.

2. **TLS + Skip certificate verification**
  - Use for self-signed server certificates on the Headscale gRPC port.
  - Setup: `Enable TLS = ON`, `Skip TLS Certificate Verification = ON`.

3. **TLS + Custom CA certificate (recommended for private PKI)**
  - Use when gRPC cert is signed by your private CA.
  - Setup: `Enable TLS = ON`, `Skip TLS Certificate Verification = OFF`, then paste CA PEM into `Custom CA Certificate (PEM)`.
  - For public CAs (e.g. ZeroSSL), keep `Custom CA Certificate (PEM)` empty to use system trust roots.
...
### TLS Certificates For Headscale gRPC

For local/manual testing of Headscale gRPC TLS, certificate helper scripts are available under `shell/certs/`:

```bash
# Self-signed server cert (panel: Enable TLS + Skip Verification)
./shell/certs/01-self-signed.sh

# Private CA + server cert (panel: Enable TLS + paste CA PEM, no Skip Verification)
./shell/certs/02-private-ca.sh
```

Generated cert artifacts are written to `shell/docker/dev/cert`, which is mounted into the headscale container at `/etc/cert` (read-only). After generating certs, update the headscale config:
...
- Tailnet test page: http://172.30.0.10/tailnet-test (after route approval)
...
4. From another tailnet device, open `http://172.30.0.10/tailnet-test`.
