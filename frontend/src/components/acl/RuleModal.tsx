...
export default function RuleModal({
  ...
  const buildSourceGroups = (): ACLOptionGroup[] => {
    ...
    groups.push({
      ...
      options: [
        { key: 'autogroup-member', label: <span><SafetyCertificateOutlined className="mr-2" />autogroup:member</span>, value: 'autogroup:member', selected: !!rule.sources?.includes('autogroup:member') },
        { key: 'autogroup-tagged', label: <span><SafetyCertificateOutlined className="mr-2" />autogroup:tagged</span>, value: 'autogroup:tagged', selected: !!rule.sources?.includes('autogroup:tagged') },
      ],
    });
    if (policy?.tagOwners && Object.keys(policy.tagOwners).length > 0) {
      groups.push({
        ...
        options: Object.keys(policy.tagOwners).map(tagName => ({
          key: `tag-src-${tagName}`, label: <span><TagOutlined className="mr-2" />{tagName}</span>, value: tagName, selected: !!rule.sources?.includes(tagName),
        })),
      });
    }
    if (Object.keys(aclGroups).length > 0) {
      groups.push({
        ...
        options: Object.keys(aclGroups).map(groupName => ({
          key: `group-src-${groupName}`, label: <span><TeamOutlined className="mr-2" />{groupName}</span>, value: groupName, selected: !!rule.sources?.includes(groupName),
        })),
      });
    }
    if (headscaleUsers.length > 0) {
      groups.push({
        ...
        options: headscaleUsers.map(user => ({
          key: `user-src-${user.name}`, label: <span><UserOutlined className="mr-2" />{user.name}@</span>, value: `${user.name}@`, selected: !!rule.sources?.includes(`${user.name}@`),
        })),
      });
    }
    if (aclDeviceOptions.length > 0) {
      groups.push({
        ...
        options: aclDeviceOptions.map(device => ({
          ...
          label: <span>{getDeviceIcon(devices.find(d => d.id === device.id) || { id: device.id, givenName: device.label, name: device.label, ipAddresses: [device.ipAddress] })} <span>{device.label}</span> <Text type="secondary" className="text-12px ml-2">{device.ipAddress}</Text></span>,
  ...
  const buildDestGroups = (): ACLOptionGroup[] => {
    ...
    groups.push({
      label: t.acl.autogroupsHeading,
      options: [
        { key: 'autogroup-internet', label: <span><SafetyCertificateOutlined className="mr-2" />autogroup:internet:*</span>, value: 'autogroup:internet:*', selected: !!rule.destinations?.includes('autogroup:internet:*') },
        { key: 'autogroup-self', label: <span><SafetyCertificateOutlined className="mr-2" />autogroup:self:*</span>, value: 'autogroup:self:*', selected: !!rule.destinations?.includes('autogroup:self:*') },
      ],
    });
    if (policy?.tagOwners && Object.keys(policy.tagOwners).length > 0) {
      groups.push({
        label: t.acl.tagsHeading,
        options: Object.keys(policy.tagOwners).map(tagName => ({
          key: `tag-dst-${tagName}`, label: <span><TagOutlined className="mr-2" />{tagName}:*</span>, value: `${tagName}:*`, selected: !!rule.destinations?.includes(`${tagName}:*`),
        })),
      });
    }
    if (Object.keys(aclGroups).length > 0) {
      groups.push({
        label: t.acl.aclGroupsHeading,
        options: Object.keys(aclGroups).map(groupName => ({
          key: `group-dst-${groupName}`, label: <span><TeamOutlined className="mr-2" />{groupName}:*</span>, value: `${groupName}:*`, selected: !!rule.destinations?.includes(`${groupName}:*`),
        })),
      });
    }
    if (policy?.hosts && Object.keys(policy.hosts).length > 0) {
      groups.push({
        ...
        options: Object.entries(policy.hosts).map(([hostName, ip]) => ({
          ...
          label: <span><GlobalOutlined className="mr-2" />{hostName}:* <Text type="secondary" className="text-12px ml-2">{ip}</Text></span>,
          value: `${hostName}:*`,
    ...
    if (resources.length > 0) {
      groups.push({
        ...
        options: resources.map(resource => ({
          ...
          label: <span><DatabaseOutlined className="mr-2" />{resource.name}:{resource.port || '*'} <Text type="secondary" className="text-12px ml-2">{resource.ip_address}</Text></span>,
        ...
        <div>
          ...
          <Space.Compact className="w-full mb-2">
            <Input
              ...
              onKeyDown={(e) => { if (e.key === 'Enter' && sourceInput) addSource(sourceInput); }}
            />
        ...
        <div>
          ...
          <Space.Compact className="w-full mb-2">
            <Input
              ...
              onKeyDown={(e) => { if (e.key === 'Enter' && destInput) addDestination(destInput); }}
            />
