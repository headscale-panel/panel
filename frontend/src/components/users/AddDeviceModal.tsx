...
export default function AddDeviceModal({ open, hsUsers, onCancel, onSuccess }: AddDeviceModalProps) {
  ...
  const buildTailscaleCommand = useMemo(() => {
    ...
    if (deployParams.hostname)
      parts.push(`--hostname=${deployParams.hostname}`);
    if (deployParams.acceptDns)
      parts.push('--accept-dns=true');
    if (deployParams.acceptRoutes)
      parts.push('--accept-routes');
    if (deployParams.advertiseExitNode)
      parts.push('--advertise-exit-node');
    if (deployParams.ssh)
      parts.push('--ssh');
    if (deployParams.shieldsUp)
      parts.push('--shields-up');
    if (deployParams.advertiseConnector)
      parts.push('--advertise-connector');
    if (deployParams.advertiseRoutes)
      parts.push(`--advertise-routes=${deployParams.advertiseRoutes}`);
    if (deployParams.advertiseTags)
      parts.push(`--advertise-tags=${deployParams.advertiseTags}`);
    if (deployParams.exitNode)
      parts.push(`--exit-node=${deployParams.exitNode}`);
    if (deployParams.exitNodeAllowLan)
      parts.push('--exit-node-allow-lan-access');
    if (deployParams.forceReauth)
      parts.push('--force-reauth');
    if (deployParams.reset)
      parts.push('--reset');
    if (!deployParams.snatSubnetRoutes)
      parts.push('--snat-subnet-routes=false');
    if (deployParams.statefulFiltering)
      parts.push('--stateful-filtering');
    if (deployParams.operator)
      parts.push(`--operator=${deployParams.operator}`);
    if (deployParams.netfilterMode && deployParams.netfilterMode !== 'on')
      parts.push(`--netfilter-mode=${deployParams.netfilterMode}`);
    return parts.join(' \\\n  ');
                                ...
                                {preAuthKeysList.map((k: any) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <div className="mt-1">
                                        {parseTimestamp(k.created_at) && (
                                          <Text type="secondary" className="text-10px mr-3">
                                            {t.devices.keyCreatedAt}
                                            :
                                            {' '}
                                            {parseTimestamp(k.created_at)!.toLocaleString()}
                                          </Text>
                                        )}
                                        {expDate && (
                                          <Text type="secondary" className="text-10px">
                                            {t.devices.keyExpiration}
                                            :
                                            {' '}
